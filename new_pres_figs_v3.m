%%Same as new_presenation_figures, but now combine position tensors along
%%dim3 instead of dim1 before calculating angles & averaging
...
%     wlist_probe_tensor_all=wlist_probe_tensor_all.*info.sig_and_pos_chans_combine_elecs;
    ...
    %% Separate tensor by probe position, store in store_tensor cell array
...
%% For each subject, find largest dim1 size
...
for j=1:size(store_wlist_probe_tensors,2)
    for i=1:size(store_wlist_probe_tensors,1)
        dim_val_sent=size(store_sent_word_tensors{i,j},1);
        dim_sizes_sent_all(i,j)=dim_val_sent;
        dim_val_wlist=size(store_wlist_word_tensors{i,j},1);
...
%% add NaN to other tensors to match max dim1
for j=1:size(store_sent_word_tensors,2)
    for i=1:size(store_sent_word_tensors,1)
        dims_needed=max_dim_sizes_sent(1,j)-size(store_sent_word_tensors{i,j},1);
        filler=NaN(dims_needed,size(store_sent_word_tensors{i,j},2),size(store_sent_word_tensors{i,j},3));
        store_sent_word_tensors{i,j}=cat(1,store_sent_word_tensors{i,j},filler);
        
        filler=NaN(dims_needed,size(store_sent_probe_tensors{i,j},2),size(store_sent_probe_tensors{i,j},3));
        store_sent_probe_tensors{i,j}=cat(1,store_sent_probe_tensors{i,j},filler);
        
        dims_needed=max_dim_sizes_wlist(1,j)-size(store_wlist_word_tensors{i,j},1);
        filler=NaN(dims_needed,size(store_wlist_word_tensors{i,j},2),size(store_wlist_word_tensors{i,j},3));
        store_wlist_word_tensors{i,j}=cat(1,store_wlist_word_tensors{i,j},filler);
        
        filler=NaN(dims_needed,size(store_wlist_probe_tensors{i,j},2),size(store_wlist_probe_tensors{i,j},3));
        store_wlist_probe_tensors{i,j}=cat(1,store_wlist_probe_tensors{i,j},filler);
    end
end

%% Combine tensors of each position
...
for j=1:size(store_sent_word_tensors,2)
    ...
    for i=1:size(store_sent_word_tensors,1)
        sent_word_combined_tensor=cat(3,sent_word_combined_tensor,store_sent_word_tensors{i,j});
        sent_probe_combined_tensor=cat(3,sent_probe_combined_tensor,store_sent_probe_tensors{i,j});
        wlist_word_combined_tensor=cat(3,wlist_word_combined_tensor,store_wlist_word_tensors{i,j});
        wlist_probe_combined_tensor=cat(3,wlist_probe_combined_tensor,store_wlist_probe_tensors{i,j});
    end
